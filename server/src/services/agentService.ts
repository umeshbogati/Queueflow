...
export const isWithinOfficeHours = (
  ...
  const now = new Date();
  const nepalTime = toZonedTime(now, TIME_ZONE);
  const currentHour = nepalTime.getHours() + nepalTime.getMinutes() / 60;
...
const ensureFreshCounters = async (): Promise<void> => {
  const date = today();
  const staleAgents = await Agent.countDocuments({
    lastResetDate: { $ne: date },
  });
  if (staleAgents > 0) {
...
export const startNoShowTimer = (
  queueId: string,
  agentId: string | null,
  timeoutMs: number = NO_SHOW_TIMEOUT_MS,
): void => {
...
export const scheduleAutoCallNext = (
  agentId: string,
  delayMs: number = AUTO_CALL_DELAY_MS,
): void => {
...
export const updateAgent = async (agentId: string, data: UpdateAgentData) => {
  ...
  if (data.counterNumber !== undefined)
    agent.counterNumber = data.counterNumber;
  ...
  if (data.maxTokensPerDay !== undefined)
    agent.maxTokensPerDay = data.maxTokensPerDay;
...
export const canServeMore = async (
  agentId: string,
  ...
  if (!isWithinOfficeHours(agent.officeStart, agent.officeEnd)) {
    return {
      allowed: false,
      reason: `Office hours are ${agent.officeStart}:00 - ${agent.officeEnd}:00`,
    };
  }
  ...
  if (agent.tokensServedToday >= agent.maxTokensPerDay) {
    return {
      allowed: false,
      reason: `Daily token limit reached (${agent.maxTokensPerDay})`,
    };
  }
...
export const agentCallNext = async (agentId: string) => {
  ...
  if (!isWithinOfficeHours(agent.officeStart, agent.officeEnd)) {
    throw new Error(
      `Office hours are ${agent.officeStart}:00 - ${agent.officeEnd}:00`,
    );
  }
...
export const agentCompleteTicket = async (agentId: string, queueId: string) => {
  ...
  if (
    updatedAgent &&
    updatedAgent.tokensServedToday < updatedAgent.maxTokensPerDay
  ) {
...
export const toAgentData = (agent: InstanceType<typeof Agent>): AgentData => {
  const userRaw = agent.user as unknown as
    | mongoose.Types.ObjectId
    | { _id: mongoose.Types.ObjectId; name: string; email?: string };
  const branchRaw = agent.branch as unknown as
    | mongoose.Types.ObjectId
    | { _id: mongoose.Types.ObjectId; name: string; location?: string };
  const departmentRaw = agent.department as unknown as
    | mongoose.Types.ObjectId
    | { _id: mongoose.Types.ObjectId; name: string; prefix?: string };

  const userData: AgentData["user"] =
    userRaw && typeof userRaw === "object" && "name" in userRaw
      ? {
          _id: (userRaw._id as mongoose.Types.ObjectId).toString(),
          name: userRaw.name,
          ...(userRaw.email !== undefined ? { email: userRaw.email } : {}),
        }
      : userRaw.toString();

  const branchData: AgentData["branch"] =
    branchRaw && typeof branchRaw === "object" && "name" in branchRaw
      ? {
          _id: (branchRaw._id as mongoose.Types.ObjectId).toString(),
          name: branchRaw.name,
          ...(branchRaw.location !== undefined
            ? { location: branchRaw.location }
            : {}),
        }
      : branchRaw.toString();

  const departmentData: AgentData["department"] =
    departmentRaw &&
    typeof departmentRaw === "object" &&
    "name" in departmentRaw
      ? {
          _id: (departmentRaw._id as mongoose.Types.ObjectId).toString(),
          name: departmentRaw.name,
          ...(departmentRaw.prefix !== undefined
            ? { prefix: departmentRaw.prefix }
            : {}),
        }
...
const toQueueData = (q: InstanceType<typeof Queue>) => {
  const branchRaw = q.branch as unknown as
    | mongoose.Types.ObjectId
    | { _id: mongoose.Types.ObjectId; name: string; location?: string };
  const departmentRaw = q.department as unknown as
    | mongoose.Types.ObjectId
    | { _id: mongoose.Types.ObjectId; name: string; prefix?: string };
  const customerRaw = q.customer as unknown as
    | mongoose.Types.ObjectId
  ...
  const customerData =
    customerRaw && typeof customerRaw === "object" && "name" in customerRaw
      ? {
          _id: (customerRaw._id as mongoose.Types.ObjectId).toString(),
  ...
  if (agentRaw) {
    if (typeof agentRaw === "object" && agentRaw.name) {
      data.agent = {
        _id: agentRaw._id?.toString() || agentRaw.toString(),
        name: agentRaw.name,
      };
